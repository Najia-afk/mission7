...
def _seed_from_compressed_csv(pg_engine, dataset_dir: str) -> bool:
    """Seed from compressed .gz CSV files (preferred method)."""
    ...
    # Load train sample
    train_chunks = pd.read_csv(train_gz, compression='gzip', chunksize=50_000)
    train_rows = _copy_chunks_to_postgres(train_chunks, pg_engine, "application_train")
    
    # Load test (full)
    test_chunks = pd.read_csv(test_gz, compression='gzip', chunksize=50_000)
    test_rows = _copy_chunks_to_postgres(test_chunks, pg_engine, "application_test")
...
def main() -> int:
    ...
    # Try compressed CSVs first (preferred), then SQLite fallback
    seeded = _seed_from_compressed_csv(pg_engine, dataset_dir) or _seed_from_sqlite(pg_engine, sqlite_path)
    if not seeded:
